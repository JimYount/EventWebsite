...
* @subpackage Asido.Driver.Imagick_Extension
*/
Class Asido_Driver_Imagick2_Ext Extends Asido_Driver {
	...
	*/
	function __resize(&$tmp, $width, $height) {
		return $tmp->target->resizeImage(
			$width, $height, Imagick::FILTER_UNDEFINED, 1);
		}
	...
	*/
	function __rotate(&$tmp, $angle, &$color) {
		...
		//
		if (($angle % 90) == 0) {
			if ($tmp->target->rotateImage(new ImagickPixel(), $angle)) {
				$size = $tmp->target->getSize();
				$tmp->image_width = $size['columns'];
				$tmp->image_height = $size['rows'];
				return true;
				}
			}
	...
	*/
	function __crop(&$tmp, $x, $y, $width, $height) {
		if (!$tmp->target->cropImage($width, $height, $x, $y)) {
			return false;
			}
		return true;
		}
	...
	*/
	function __canvas($width, $height, &$color) {
		...
		$t->target->newImage($width, $height, new ImagickPixel("rgb($r, $g, $b)"));
	...
	*/
	function __tmpimage(&$handler, $filename=null) {
		...
		$handler->writeImage($handler, $filename);
			// ^
			// PNG: no pixel losts
	...
	*/
	function __open(&$tmp) {
		$tmp->source = new Imagick();
		$error_open = !($tmp->source->readImage(realpath($tmp->source_filename)));
		$error_open &= !($tmp->target = $tmp->source->clone());
			
		// get width & height of the image
		//
		if (!$error_open) {
			$size = $tmp->source->getSize();
			$tmp->image_width = $size['columns'];
			$tmp->image_height = $size['rows'];
			}
	...
	*/
	function __write(&$tmp) {
		...
		if ($tmp->save) {
			...
			// convert, then save
			//
			$tmp->target->setType($this->__mime_map[$tmp->save]);
